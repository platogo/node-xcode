...
pbxProject.prototype.addPbxVariantGroup = function(filePathsArray, file, sourceTree) {
    var groups = this.hash.project.objects['PBXVariantGroup'],
        name = file.basename,
        pbxGroupUuid = file.fileRef
        commentKey = f("%s_comment", pbxGroupUuid),
        pbxGroup = {
            isa: 'PBXVariantGroup',
            children: [],
    ...
    for (var key in fileReferenceSection) {
        ...
        // ignore base file of variant group to link default language properly
        if(fileReference.path !== file.path && fileReference.path !== "\"" + file.path + "\"") {
            filePathToReference[fileReference.path] = { fileRef: fileReferenceKey, basename: fileReferenceSection[key] };
        }
    }
    ...
    // create PBXVariantGroup if it does not exist
    if(!groups) {
        groups = {}
        this.hash.project.objects['PBXVariantGroup'] = groups
    }
...
pbxProject.prototype.removePbxVariantGroup = function (groupName) {
    var section = this.hash.project.objects['PBXVariantGroup'],
        key, itemKey;
...
pbxProject.prototype.replaceKnownRegions = function(regionsArray) {
    var group = this.getPBXObject('PBXProject')
    var key = Object.keys(group)[0]
    group[key]['knownRegions'] = regionsArray
}
